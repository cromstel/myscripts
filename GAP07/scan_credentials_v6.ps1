...
.DESCRIPTION
    ...
    and ETA. Program Files, Windows, $Recycle.Bin, System Volume Information, PerfLogs,
    %userprofile%\AppData (any user), Temp/tmp folders, and any OS "system" or "hidden"
    attribute file/folder are skipped entirely / never traversed. EFS-encrypted and temp
    files (.tmp/.bak/Office lock files) are also skipped per-file. Disable via config:
    ExcludeSystemPaths / ExcludeEncryptedFiles / ExcludeTempPaths (all default true). The
    scan requests Windows' true background process mode (lowers CPU, disk I/O, and memory
    priority together; falls back to BelowNormal CPU priority if unavailable) and throttles
    between files (ThrottleMs) so it doesn't impact the host - this applies to every drive
    scanned, including the system drive (C:).

    Scan targets default to EXPLICIT disk drive(s)/directory(ies) only - i.e. -ScanPath
    arguments and the config file's AdditionalScanPaths. Phase 1 still discovers and
    ACL-audits SMB shares (that's a separate, always-on finding), but discovered shares are
    NOT automatically added as content-scan targets - set ScanDiscoveredShares=true in
    config, or accept them when prompted in interactive mode, to include them. System/
    administrative shares (C$, ADMIN$, IPC$, print$, etc.) require their own explicit
    opt-in (ExcludeSystemShares=false in config, or the interactive Y/N prompt at startup).
    ...
    Two modes:
      interactive - prompts for drives/directories (and optional shares), then scans
      automated   - scans explicit targets non-interactively (for Ansible Tower)
...
.NOTES
    Dab17 Security Audit Framework
    Version       : 2.3.1
    Last Modified : 2026-06-25
    Author        : [pending - tell Claude your name to fill this in]
    Company       : [pending - tell Claude your company to fill this in]
#>
Param(
...
try {
    # PROCESS_MODE_BACKGROUND_BEGIN (Vista+) lowers CPU, disk I/O, AND memory priority
    # together - a stronger guarantee against impacting a live file server than the
    # CPU-only BelowNormal priority class. Falls back to BelowNormal if unavailable.
    Add-Type -Name NativeProcessPriority -Namespace WinAuditReport -MemberDefinition @'
[System.Runtime.InteropServices.DllImport("kernel32.dll", SetLastError = true)]
public static extern System.IntPtr GetCurrentProcess();

[System.Runtime.InteropServices.DllImport("kernel32.dll", SetLastError = true)]
public static extern bool SetPriorityClass(System.IntPtr hProcess, uint dwPriorityClass);
'@ -ErrorAction Stop

    $PROCESS_MODE_BACKGROUND_BEGIN = 0x00100000
    $currentProcHandle = [WinAuditReport.NativeProcessPriority]::GetCurrentProcess()
    $backgroundModeOk = [WinAuditReport.NativeProcessPriority]::SetPriorityClass($currentProcHandle, $PROCESS_MODE_BACKGROUND_BEGIN)
    if (-not $backgroundModeOk) {
...
$DefaultConfig = @{
    ...
    ThrottleMs            = 5
    AdditionalScanPaths   = @()
    ExcludeSystemShares   = $true   # Admin/system SMB shares (C$, ADMIN$, IPC$, print$, etc.)
    ...
    ExcludeTempPaths      = $true   # Skip Temp/tmp/Temporary Internet Files folders and .tmp/.bak/Office-lock files
    ScanDiscoveredShares  = $false  # If false (default), Phase 1 still discovers/audits shares, but their
                                     # paths are NOT auto-added as content-scan targets - only explicit
                                     # -ScanPath / AdditionalScanPaths (selected drives/directories) are scanned.
}
...
function Load-Config {
    ...
    if ($Path -and (Test-Path $Path)) {
        try {
            ...
            $exclTemp     = if ($null -ne $raw.ExcludeTempPaths) { $raw.ExcludeTempPaths } elseif ($null -ne $raw.exclude_temp_paths) { $raw.exclude_temp_paths } else { $null }
            $scanShares   = if ($null -ne $raw.ScanDiscoveredShares) { $raw.ScanDiscoveredShares } elseif ($null -ne $raw.scan_discovered_shares) { $raw.scan_discovered_shares } else { $null }
            ...
            if ($null -ne $exclTemp)     { $cfg.ExcludeTempPaths      = [bool]$exclTemp }
            if ($null -ne $scanShares)   { $cfg.ScanDiscoveredShares  = [bool]$scanShares }
        } catch {
...
# locations (Windows, Program Files), per-user AppData (any username, Roaming/Local/
# LocalLow), wherever they appear under a scan root, e.g. when a whole drive letter
# (C:\) is scanned.
$Script:ExcludedPathRegex = [regex]::new(
    '\\(Windows|WindowsApps|Program Files|Program Files \(x86\)|\$Recycle\.Bin|System Volume Information|Recovery|PerfLogs|MSOCache)\\|\\Users\\[^\\]+\\AppData\\',
    [System.Text.RegularExpressions.RegexOptions]::IgnoreCase -bor [System.Text.RegularExpressions.RegexOptions]::Compiled)

# Temp directories - kept separate from the system-path set above so the two can be
# toggled independently (ExcludeSystemPaths vs ExcludeTempPaths).
$Script:ExcludedTempDirNames = [System.Collections.Generic.HashSet[string]]::new(
    [string[]]@('Temp', 'tmp', 'Temporary Internet Files', '~tmp'),
    [System.StringComparer]::OrdinalIgnoreCase)

$Script:ExcludedTempPathRegex = [regex]::new(
    '\\(Temp|tmp|Temporary Internet Files|AppData\\Local\\Temp|AppData\\LocalLow\\Temp)\\',
    [System.Text.RegularExpressions.RegexOptions]::IgnoreCase -bor [System.Text.RegularExpressions.RegexOptions]::Compiled)

# Individual temp *files* by extension/name, checked regardless of which folder they're
# in (e.g. a stray .tmp file sitting in an otherwise-scanned share).
$Script:TempFileExtSet = [System.Collections.Generic.HashSet[string]]::new(
    [string[]]@('.tmp', '.temp', '.bak', '.~tmp'),
    [System.StringComparer]::OrdinalIgnoreCase)

function Test-PathExcluded {
    param([string]$FullPath, [string]$LeafName, [bool]$ExcludeSystemPaths = $true, [bool]$ExcludeTempPaths = $true)
    ...
    if ($ExcludeTempPaths) {
        if ($LeafName -and $Script:ExcludedTempDirNames.Contains($LeafName)) { return $true }
        if ($Script:ExcludedTempPathRegex.IsMatch("$FullPath\")) { return $true }
    }
    return $false
}

function Test-IsTempFile {
    param([string]$Name, [string]$Extension)
    if ($Script:TempFileExtSet.Contains($Extension)) { return $true }
    if ($Name -like '~$*') { return $true }   # Office lock files, e.g. ~$Budget.xlsx
...
$Script:Stats = @{
    ...
    DirsExcluded = 0; FilesExcludedSystem = 0; FilesExcludedEncrypted = 0; FilesExcludedTemp = 0; FilesExcludedHidden = 0
}
...
function Select-ScanTargets {
    param(
        [System.Collections.Generic.List[string]]$ExplicitPaths,
        [System.Collections.Generic.List[string]]$DiscoveredPaths,
        [bool]$IncludeSystemShares
    )

    $Paths = [System.Collections.Generic.List[string]]::new()
    foreach ($p in $ExplicitPaths) { [void]$Paths.Add($p) }

    Write-Host ""
    Write-Host "=== Selected Disk Drive(s) / Directory(ies) ===" -ForegroundColor Cyan
    if ($Paths.Count -eq 0) {
        Write-Host "  (none yet - this scan defaults to explicit drives/directories, not auto-discovered shares)"
    } else {
    ...
    try {
        $localDrives = [System.IO.DriveInfo]::GetDrives() |
            Where-Object { $_.IsReady -and $_.DriveType -in @('Fixed', 'Removable', 'Network') }
        if ($localDrives) {
            Write-Host ""
            Write-Host "  Available local drives:"
            foreach ($d in $localDrives) {
                $freeGb = [math]::Round($d.AvailableFreeSpace / 1GB, 1)
                $totalGb = [math]::Round($d.TotalSize / 1GB, 1)
                Write-Host "    $($d.Name)  ($($d.DriveType), $freeGb GB free / $totalGb GB)"
            }
        }
    } catch {}

    Write-Host ""
    Write-Host "Enter disk drive(s) and/or directory(ies) to scan (comma-separated, e.g. C:\, D:\Data), or press Enter to skip:"
    $addInput = Read-Host
    if ($addInput) {
    ...
    if ($DiscoveredPaths -and $DiscoveredPaths.Count -gt 0) {
        Write-Host ""
        Write-Host "=== Discovered Shares (not included by default) ===" -ForegroundColor Cyan
        $shareNote = if ($IncludeSystemShares) { 'includes system/administrative shares' } else { 'system/administrative shares excluded' }
        Write-Host "  Found $($DiscoveredPaths.Count) share(s) via SMB discovery ($shareNote)."
        for ($i = 0; $i -lt $DiscoveredPaths.Count; $i++) {
            Write-Host "  [$($i+1)] $($DiscoveredPaths[$i])"
        }
        Write-Host ""
        Write-Host "Enter numbers to ALSO include in this scan (comma-separated), or press Enter to skip:"
        $shareInput = Read-Host
        if ($shareInput) {
            foreach ($n in ($shareInput -split ',')) {
                $n = $n.Trim()
                if ($n -match '^\d+$') {
                    $idx = [int]$n - 1
                    if ($idx -ge 0 -and $idx -lt $DiscoveredPaths.Count) {
                        [void]$Paths.Add($DiscoveredPaths[$idx])
                        Write-Host "  Added: $($DiscoveredPaths[$idx])" -ForegroundColor Green
                    }
                }
            }
        }
        ...
        try {
            foreach ($f in $dirInfo.EnumerateFiles()) {
                try {
                    ...
                    if ($ExcludeSystemPaths -and ($attrs -band [System.IO.FileAttributes]::Hidden)) {
                        $Script:Stats.FilesExcludedHidden++; continue
                    }
                    if ($ExcludeEncryptedFiles -and ($attrs -band [System.IO.FileAttributes]::Encrypted)) {
                        $Script:Stats.FilesExcludedEncrypted++; continue
                    }
                    if ($ExcludeTempPaths -and (Test-IsTempFile -Name $f.Name -Extension $f.Extension)) {
                        $Script:Stats.FilesExcludedTemp++; continue
                    }
                    $results.Add($f)
                } catch {
                    $Script:Stats.AccessErrors++
            ...
            try {
                foreach ($d in $dirInfo.EnumerateDirectories()) {
                    try {
                        ...
                        if (Test-PathExcluded -FullPath $d.FullName -LeafName $d.Name -ExcludeSystemPaths $ExcludeSystemPaths -ExcludeTempPaths $ExcludeTempPaths) {
                            $Script:Stats.DirsExcluded++; continue
                        }
                        if ($ExcludeSystemPaths -and ($d.Attributes -band ([System.IO.FileAttributes]::System -bor [System.IO.FileAttributes]::Hidden))) {
                            $Script:Stats.DirsExcluded++; continue
                        }
...
function Invoke-FullScan {
    ...
    foreach ($scanRoot in $ScanPaths) {
        ...
        $files = Get-EligibleFiles -RootPath $scanRoot -MaxDepth $Config.MaxDepth `
            ...
            -ExcludeTempPaths $Config.ExcludeTempPaths
        $total = $files.Count
...
function Show-Banner {
    ...
    Write-Host ""
    Write-Host $border
    Write-Host "  CREDENTIAL EXPOSURE AUDIT PLATFORM - v2.3.1"
    Write-Host $border
    ...
    Write-Host "  ExclSysShares      : $($Config.ExcludeSystemShares) (C$/ADMIN$/IPC$/print$/etc.)"
    Write-Host "  ExclSysPaths       : $($Config.ExcludeSystemPaths) (Windows/Program Files/AppData/hidden+system attrib/etc.)"
    Write-Host "  ExclEncrypted      : $($Config.ExcludeEncryptedFiles)"
    Write-Host "  ExclTempPaths      : $($Config.ExcludeTempPaths) (Temp/tmp/.tmp/.bak/Office lock files)"
    Write-Host "  ScanDiscoveredShr  : $($Config.ScanDiscoveredShares) (focus is explicit drives/dirs unless true)"
...
function Write-SummaryFile {
    ...
    [void]$sb.AppendLine("Files excluded:   $($Script:Stats.FilesExcludedSystem) (system attrib), $($Script:Stats.FilesExcludedHidden) (hidden), $($Script:Stats.FilesExcludedEncrypted) (encrypted), $($Script:Stats.FilesExcludedTemp) (temp)")
    ...
    Write-Host $border
    Write-Host "  Duration           : $($elapsed.ToString('hh\:mm\:ss')) (hh:mm:ss)"
    Write-Host "  Targets scanned    : $($Targets.Count)"
    Write-Host "  Files scanned      : $($Script:Stats.FilesScanned)"
    Write-Host "  Total detections   : $total (HIGH:$($Script:Stats.DetectionHigh) MEDIUM:$($Script:Stats.DetectionMedium) INFO:$($Script:Stats.DetectionInfo))"
    Write-Host $border
    Write-Host ""
...
function Main {
    ...
    if ([string]::IsNullOrEmpty($OutputPath)) {
        ...
        $Script:OutputDir = Join-Path $scriptDir "WinAuditReport_${Hostname}_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
    } else {
        if ($OutputPath -match '\.ps1$') {
            Write-Warning "-OutputPath is '$OutputPath' - that looks like a script filename, not a folder."
            Write-Warning "This usually means the script's own path has a space and wasn't quoted, e.g.:"
            Write-Warning "  WRONG : .\some script.ps1 -Mode automated ..."
            Write-Warning "  RIGHT : & '.\some script.ps1' -Mode automated ..."
            Write-Warning "Continuing anyway, but check your command line if this isn't intentional."
        }
        $Script:OutputDir = $OutputPath
    }

    # Always resolve to an absolute path, anchored to PowerShell's own location ($PWD) -
    # NOT .NET's Environment.CurrentDirectory, which can silently differ from $PWD in
    # Windows PowerShell 5.1 after Set-Location/cd. Several parts of this script call raw
    # .NET file APIs (StreamWriter, DirectoryInfo, etc.) directly; those resolve relative
    # paths against Environment.CurrentDirectory, so a relative $OutputDir can succeed in
    # New-Item (PowerShell-native, uses $PWD) but then fail in those .NET calls with a
    # DirectoryNotFoundException pointing somewhere unexpected (e.g. the user's profile
    # folder instead of the script's actual working directory).
    if (-not [System.IO.Path]::IsPathRooted($Script:OutputDir)) {
        $Script:OutputDir = Join-Path $PWD.Path $Script:OutputDir
    }
    ...
    try {
        # Explicit targets are the default focus of the scan: selected disk drive(s) and/or
        # directory(ies) supplied via -ScanPath or the config file's AdditionalScanPaths.
        # Auto-discovered SMB shares are NOT merged in automatically (see ScanDiscoveredShares
        # below) - they're offered as an opt-in, not a default.
        $explicitPaths = [System.Collections.Generic.List[string]]::new()
        $seenPaths = [System.Collections.Generic.HashSet[string]]::new([System.StringComparer]::OrdinalIgnoreCase)
        foreach ($ap in $config.AdditionalScanPaths) {
            if ($ap -and $seenPaths.Add($ap)) { $explicitPaths.Add($ap) }
        }
        foreach ($sp in $ScanPath) {
            if ($sp -and $seenPaths.Add($sp)) { $explicitPaths.Add($sp) }
        }

        # Whether to include system/administrative shares (C$, ADMIN$, IPC$, print$, etc.)
        # at all - in headless/automated mode this is purely config-driven; in interactive
        # mode the operator is asked directly so they don't need to edit a JSON file.
        $includeSystemShares = -not $config.ExcludeSystemShares

        if ($Mode -eq 'interactive') {
            Write-Host ""
            $sysAnswer = Read-Host "Include system/administrative shares (C$, ADMIN$, IPC$, etc.) in discovery and scanning? [y/N]"
            $includeSystemShares = ($sysAnswer -eq 'y' -or $sysAnswer -eq 'Y')
        }

        $sharePaths = @()
        try { $sharePaths = Invoke-ShareDiscovery -Hostname $Hostname -ExcludeSystem (-not $includeSystemShares) }
        catch { Write-Error -ErrorAction Continue -Message "Phase 1 failed: $($_.Exception.Message)" }

        $remotePaths = @()
        try { $remotePaths = Invoke-RemoteShareDiscovery -Paths @($explicitPaths) -ExcludeSystem (-not $includeSystemShares) }
        catch { Write-Error -ErrorAction Continue -Message "Remote share discovery failed: $($_.Exception.Message)" }

        # Discovered shares are tracked separately from explicit targets - they're a
        # candidate pool, not an automatic addition, unless ScanDiscoveredShares is set.
        $discoveredPaths = [System.Collections.Generic.List[string]]::new()
        foreach ($sp in $sharePaths) {
            if ($sp -and -not $seenPaths.Contains($sp)) { [void]$seenPaths.Add($sp); $discoveredPaths.Add($sp) }
        }
        foreach ($rp in $remotePaths) {
            if ($rp -and -not $seenPaths.Contains($rp)) { [void]$seenPaths.Add($rp); $discoveredPaths.Add($rp) }
        }

        if ($Mode -eq 'interactive') {
            $allScanPaths = Select-ScanTargets -ExplicitPaths $explicitPaths -DiscoveredPaths $discoveredPaths -IncludeSystemShares $includeSystemShares
        } else {
            $allScanPaths = [System.Collections.Generic.List[string]]::new()
            foreach ($p in $explicitPaths) { [void]$allScanPaths.Add($p) }

            if ($config.ScanDiscoveredShares) {
                foreach ($dp in $discoveredPaths) { [void]$allScanPaths.Add($dp) }
            } elseif ($discoveredPaths.Count -gt 0) {
                Write-Host "`n  ($($discoveredPaths.Count) discovered share(s) NOT included - set ScanDiscoveredShares=true in config to include them)"
            }
        ...
        if ($targetArray.Count -eq 0) {
            ...
            Write-Host ""
            Write-Host "    By default this scan focuses on EXPLICIT drives/directories, not" -ForegroundColor Yellow
            Write-Host "    auto-discovered shares. Targets come from:" -ForegroundColor Yellow
            Write-Host "      1) -ScanPath arguments (e.g. -ScanPath C:\, D:\Data)"
            Write-Host "      2) Config file 'AdditionalScanPaths' (-ConfigPath)"
            Write-Host "      3) Discovered SMB shares, only if ScanDiscoveredShares=true in config"
            Write-Host "         or selected interactively"
            Write-Host ""
