...
.DESCRIPTION
    ...
    Live Write-Progress bar during Phase 2/3 (files scanned / total, current file, running
    detection counts). Program Files, Windows, $Recycle.Bin, System Volume Information and
    other OS-noise directories are skipped entirely (never traversed), and EFS-encrypted or
...
.NOTES
    Dab17 Security Audit Framework v2.1.0
#>
Param(
...
$PatternBank = @{
    en = '(password|passwd|pwd|credential|secret|api[_\-]?key|apikey|token|auth[_\-]?(token|key|secret)|private[_\-]?key|access[_\-]?key|client[_\-]?secret|jwt[_\-]?secret|db[_\-]?pass(word)?|connection[_\-]?string)\s*[:=]\s*[''"]?[^\s''"]{3,}'
    fr = '(mot[_ ]de[_ ]passe|mdp|identifiant|cl[e\u00e9]|jeton|authentification|acc[\u00e8e]s)\s*[:=]\s*[''"]?[^\s''"]{3,}'
    es = '(contrase[\u00f1n]a|clave|secreto|credencial|llave|acceso|autenticaci[o\u00f3]n)\s*[:=]\s*[''"]?[^\s''"]{3,}'
    pt = '(senha|chave|segredo|autentica[c\u00e7][a\u00e3]o)\s*[:=]\s*[''"]?[^\s''"]{3,}'
    de = '(passwort|kennwort|geheimnis|schl[u\u00fc]ssel|zugang|berechtigung|zugangsdaten)\s*[:=]\s*[''"]?[^\s''"]{3,}'
    it = '(chiave|segreto|credenziale)\s*[:=]\s*[''"]?[^\s''"]{3,}'
    nl = '(wachtwoord|sleutel|geheim|toegang|inloggegevens)\s*[:=]\s*[''"]?[^\s''"]{3,}'
    tr = '([s\u015f]ifre|anahtar|gizli|kimlik)\s*[:=]\s*[''"]?[^\s''"]{3,}'
}
...
$TextExtSet = [System.Collections.Generic.HashSet[string]]::new(
    [string[]]@(
        '.txt','.md','.log','.cfg','.cnf','.conf','.ini','.xml','.json','.yaml','.yml','.env','.env.*',
    ...
    foreach ($lang in $CompiledPatterns.Keys) {
        ...
        foreach ($m in $matches) {
            ...
            Write-CsvRow -Writer $Writer -Row @{
                ...
                detection_type=$DetectionType; matched_pattern=$m.Groups[1].Value
                severity=$severity; context_snippet=$snippet; language=$lang
            }
            if ($severity -eq 'HIGH') { $Script:Stats.DetectionHigh++ } else { $Script:Stats.DetectionMedium++ }
        }
    }
}
...
function Invoke-FullScan {
    ...
    foreach ($scanRoot in $ScanPaths) {
        ...
        Write-Progress -Activity $activity -Status 'Enumerating eligible files...' -PercentComplete 0
...
function Show-Banner {
    ...
    Write-Host ""
    Write-Host "[*] Credential Exposure Audit v2.1.0"
    ...
    Write-Host "    Extra Paths    : $($Config.AdditionalScanPaths -join ', ')"
    Write-Host "    Start Time     : $([datetime]::UtcNow.ToString('o'))"
    Write-Host ""
}
...
function Write-SummaryFile {
    ...
    [void]$sb.AppendLine("  INFO:           $($Script:Stats.DetectionInfo)")
    [void]$sb.AppendLine("------------------------------------------")
...
function Main {
    ...
    if ([string]::IsNullOrEmpty($OutputPath)) {
        ...
        $Script:OutputDir = Join-Path $scriptDir "audit_scan_${Hostname}_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
    } else {
