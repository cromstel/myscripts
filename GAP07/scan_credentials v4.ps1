...
.DESCRIPTION
    ...
    and ETA. Program Files, Windows, $Recycle.Bin, System Volume Information, Temp/tmp
    folders and other OS-noise directories are skipped entirely (never traversed), and
    EFS-encrypted, OS "system"-attribute, or temp files (.tmp/.bak/Office lock files) are
    skipped per-file. Disable via config: ExcludeSystemPaths / ExcludeEncryptedFiles /
    ExcludeTempPaths (all default true). The scan also runs at BelowNormal process priority
    and throttles between files (ThrottleMs) to avoid impacting the host.
...
.NOTES
    ...
    Author        : Samuel Lamptey
    Company       : BNP Parabas Nordic
#>
Param(
...
$Script:Stats = @{
    ...
    DirsExcluded = 0; FilesExcludedSystem = 0; FilesExcludedEncrypted = 0; FilesExcludedTemp = 0
}
            ...
            try {
                foreach ($d in $dirInfo.EnumerateDirectories()) {
                    try {
                        ...
                        if ($ExcludeSystemPaths -and ($d.Attributes -band [System.IO.FileAttributes]::System)) {
                            $Script:Stats.DirsExcluded++; continue
                        }
...
function Write-SummaryFile {
    ...
    [void]$sb.AppendLine("Files excluded:   $($Script:Stats.FilesExcludedSystem) (system attrib), $($Script:Stats.FilesExcludedEncrypted) (encrypted), $($Script:Stats.FilesExcludedTemp) (temp)")
